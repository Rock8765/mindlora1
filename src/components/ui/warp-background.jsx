...
export const WarpBackground = ({
  ...
  return (
    <div className={cn("relative rounded border p-20", className)} {...props}>
      ...
      >
        <div className={cn(sideBase, "[transform-origin:50%_0%] [transform:rotateX(-90deg)] [width:100cqi]")}>
          {topBeams.map((beam, index) => (
            <Beam key={`top-${index}`} width={`${beamSize}%`} x={`${beam.x * beamSize}%`} delay={beam.delay} duration={beamDuration} />
          ))}
        </div>
        <div className={cn(sideBase, "top-full [transform-origin:50%_0%] [transform:rotateX(-90deg)] [width:100cqi]")}>
          {bottomBeams.map((beam, index) => (
            <Beam key={`bottom-${index}`} width={`${beamSize}%`} x={`${beam.x * beamSize}%`} delay={beam.delay} duration={beamDuration} />
          ))}
        </div>
        <div className={cn(sideBase, "left-0 top-0 [transform-origin:0%_0%] [transform:rotate(90deg)_rotateX(-90deg)] [width:100cqh]")}>
          {leftBeams.map((beam, index) => (
            <Beam key={`left-${index}`} width={`${beamSize}%`} x={`${beam.x * beamSize}%`} delay={beam.delay} duration={beamDuration} />
          ))}
        </div>
        <div className={cn(sideBase, "right-0 top-0 [width:100cqh] [transform-origin:100%_0%] [transform:rotate(-90deg)_rotateX(-90deg)]")}>
