    ...
    <div className="container-xl">
      <div className="grid items-center gap-14 lg:grid-cols-12">
        <div className="lg:col-span-6">
          ...
          >
            Share your hiring requirement. Mindlora interviews every candidate across HR, technical, and coding rounds, then hands you a ranked shortlist with full scorecards.
          </motion.p>

          <motion.div initial="hidden" animate="show" custom={3} variants={fadeUp} className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <MagneticButton>
              <Button asChild size="lg" className="h-12 rounded-xl bg-gradient-primary px-6 text-base shadow-xs transition-all hover:shadow-glow">
                 <Link to="/book-meeting"><CalendarCheck className="h-5 w-5" /> Get Quotation</Link>
              </Button>
            </MagneticButton>
            <Button asChild size="lg" variant="ghost" className="h-12 rounded-xl px-4 text-base">
              <Link to="/products">See a sample scorecard <ArrowRight className="h-4 w-4" /></Link>
            </Button>
          </motion.div>

          <motion.div initial="hidden" animate="show" custom={4} variants={fadeUp} className="mt-6 flex items-center gap-2 text-sm text-muted-foreground">
            <ShieldCheck className="h-4 w-4 text-success" /> Custom interview plan · Quotation within 24 hours
          </motion.div>
        </div>
